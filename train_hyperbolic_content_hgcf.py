#!/usr/bin/env python3
"""
Content-aware HGCF (분자 GNN + 노트 → 포앵카레 랭킹).

하이퍼볼릭 CF 스타일: 동일 입력·데이터·평가, 포앵카레 볼에서 BPR 학습.
실행: hyperbolic_model 디렉터리에서
  python train_hyperbolic_content_hgcf.py --load_dataset results/checkpoints/datasets
결과: recbole_baselines/results/recbole_ContentHGCF.json

에폭마다 Val HR@10이 일정하면 z_recipe 붕괴 가능 → --lr 2e-4 또는 1e-4 시도.
"""
...
from model.hyperbolic_content_hgcf import ContentHGCF
...
def main():
    p = argparse.ArgumentParser(description="Content HGCF (분자 GNN + 노트, 포앵카레)")
    ...
    p.add_argument("--lr", type=float, default=2e-4, help="학습률 (붕괴 시 1e-4)")
    p.add_argument("--embed_dim", type=int, default=128)
    p.add_argument("--curvature", type=float, default=0.4, help="Poincare ball curvature c (HGCF 기본 0.4)")
    ...
    best_state = None
    patience = 0
    print("  💡 에폭마다 Val HR@10이 일정하면 z_recipe 붕괴 가능 → --lr 2e-4 또는 1e-4 시도")
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}", leave=False):
            ...
            if getattr(args, "diagnose", False):
                ...
                if epoch == 0 and n_b == 0:
                    ...
                    print(f"[진단 HGCF] Epoch 1 첫 배치 | Recipe 거리(z[0]-z[1]): {d01:.4f} | z_recipe batch std: {z_std:.6f} | z L2 평균: {z_norm_mean:.4f}")
                    print(f"            target_tensor shape: {target_tensor.shape} | 배치 내 서로 다른 정답 blender 수: {len(uniq)}")
                elif n_b == 0:
                    print(f"[진단 HGCF] Epoch {epoch+1} 첫 배치 | Recipe 거리: {d01:.4f} | z std: {z_std:.6f} | z L2: {z_norm_mean:.4f}")
    ...
    out_path = results_dir / "recbole_ContentHGCF.json"
