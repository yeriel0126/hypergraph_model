...
from eval_metrics import recall_at_k, mrr_at_k
...
def train(model):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            recall, ndcg, mrr = results[0], results[1], results[2]
            if args.log:
                log.write('Test:{:3d}\t{:.3f}\t{:.4f}\t{:.3f}\t{:.4f}\t{:.4f}\n'.format(
                    epoch + 1, recall[1], recall[2], ndcg[1], ndcg[2], mrr))
            else:
                # recall@1,5,10,20,50 / ndcg@1,5,10,20,50 / mrr (parse_hgcf_stdout에서 사용)
                print('\t'.join([str(round(x, 4)) for x in recall]))
                print('\t'.join([str(round(x, 4)) for x in ndcg]))
                print(str(round(mrr, 4)))
...
def eval_rec(pred_matrix, data):
    ...
    # recall@1,5,10,20,50 (hit_rate@1 = recall@1)
    recall = []
    for k in [1, 5, 10, 20, 50]:
        recall.append(recall_at_k(data.test_dict, pred_list, k))

    all_ndcg = ndcg_func([*data.test_dict.values()], pred_list)
    ndcg = [all_ndcg[x-1] for x in [1, 5, 10, 20, 50]]

    mrr = mrr_at_k(data.test_dict, pred_list, topk=topk)
