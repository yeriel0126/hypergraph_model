"""
Training script for Hierarchical Hyperbolic Hypergraph model (기본 제안 모델, GNN 기반).

분자 입력: SMILES → GNN (또는 프리컴퓨팅 임베딩). 지문(ECFP/Morgan) 전용은
train_hyperbolic_hypergraph_fingerprint.py 를 사용하세요.
"""
...
def evaluate_model(
    ...
    return_z_recipe_stats: bool = False,
) -> Dict[str, float]:
    """
    ...
    recipe_norm_list = []
    # 검증 첫 배치 z_recipe 붕괴 진단 (return_z_recipe_stats 시)
    z_recipe_mean_pairwise_dist = None
    z_recipe_std_norm = None
    z_stats_done = False
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            if 'precomputed_mol_embs' in batch:
                ...
                if 'precomputed_mol_embs' in sig.parameters:
                    forward_kw['precomputed_mol_embs'] = batch['precomputed_mol_embs'].to(device, non_blocking=use_non_blocking)
            z_recipe = model(**forward_kw)
            
            # z_recipe norm 수집 (collapse/boundary 감지용)
            recipe_norm_list.append(z_recipe.norm(dim=1).cpu())
            # 검증 첫 배치에서 z_recipe 분산 진단 (모든 레시피가 같은 벡터면 HR@10 고정 원인)
            if return_z_recipe_stats and not z_stats_done and z_recipe.size(0) >= 2:
                n = z_recipe.size(0)
                dists = torch.cdist(z_recipe, z_recipe, p=2)
                triu = dists.triu(diagonal=1)
                cnt = (n * (n - 1)) // 2
                z_recipe_mean_pairwise_dist = (triu.sum() / cnt).item()
                z_recipe_std_norm = z_recipe.norm(dim=1).std().item()
                z_stats_done = True
        ...
        if z_recipe_mean_pairwise_dist is not None:
            out['z_recipe_mean_pairwise_dist'] = z_recipe_mean_pairwise_dist
        if z_recipe_std_norm is not None:
            out['z_recipe_std_norm'] = z_recipe_std_norm
        return out
...
def main():
    ...
    default_output_dir = script_dir / 'results' / 'checkpoints'  # 스크립트 위치 기준
    ...
    parser.add_argument('--precomputed_mol_embs', type=str, default=None,
                       help='프리컴퓨팅된 분자 임베딩 .pt 경로 (기본: 미사용, GNN end-to-end). 지정 시 해당 .pt 사용')
    ...
    # Create datasets (기본 제안 모델: GNN 또는 프리컴퓨팅, 지문 미사용)
    molecule_dropout_rate = getattr(args, 'molecule_dropout_rate', 0.15)
    precomputed_path = getattr(args, 'precomputed_mol_embs', None)
    if precomputed_path:
    ...
    # 프리컴퓨팅 사용 시 GNN 파인튜닝 비활성화: requires_grad=False로 고정 (파일이 있을 때만)
    # 지문 모드(use_fingerprint)에서는 smiles_encoder가 None이므로 스킵
    use_precomputed = bool(getattr(args, 'precomputed_mol_embs', None)) and Path(args.precomputed_mol_embs).exists()
    if use_precomputed and hasattr(model, 'smiles_encoder') and model.smiles_encoder is not None:
        for p in model.smiles_encoder.parameters():
            p.requires_grad = False
        n_frozen = sum(p.numel() for p in model.smiles_encoder.parameters())
        print(f"   🔒 GNN 고정 (파인튜닝 없음): smiles_encoder {n_frozen:,} 파라미터 requires_grad=False")
    ...
    if use_riemannian:
        optimizer = RiemannianAdam(
            trainable_params if use_precomputed else model.parameters(),
